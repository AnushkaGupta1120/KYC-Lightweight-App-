...
import {KYCDataManager} from '../services/KYCDataManager';
...
const SuccessScreen = ({navigation}) => {
  ...
  const loadData = async () => {
    ...
    const data = await KYCDataManager.getKYCData();
    setKYCData(data);
  };
  ...
  const handleFinish = async () => {
    ...
    await KYCDataManager.saveKYCData('completion', {
      completedAt: Date.now(),
      language: currentLanguage,
      method: kycData?.kycMethod || 'unknown',
    });
  ...
  const getCompletionSummary = () => {
    ...
    const documents = Object.keys(kycData).filter(key => 
      key.includes('document') || key === 'aadhaar' || key === 'pan'
    );

    return {
      documentsVerified: documents.length,
      faceAuthCompleted: !!kycData.faceAuth,
      method: kycData.kycMethod === 'digilocker' ? 'DigiLocker' : 'Photo Upload',
